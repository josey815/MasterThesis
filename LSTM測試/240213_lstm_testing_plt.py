...
for i in range(242): #242
  ...
  df = pd.DataFrame({'remaining time':string[:,0], 'pred_deltaA':pred_deltaA[:,0], 'real_deltaA':real_deltaA[:,0], 'pred_deltaB':pred_deltaB[:,0], 'real_deltaB':real_deltaB[:,0]})
  print(df.shape)
  df.to_csv(folder+'df' + str(i) + '.csv', index=False)

Rowname_output = ['remaining time', 'pred_deltaA', 'real_deltaA', 'pred_deltaB', 'real_deltaB']
...
for i in range(split):
    ...
    for j in range(242):
        x.append(output[i*time_length+24, j*5+2]) #true
        y.append(output[i*time_length+24, j*5+1]) #pred
...
for i in range(5):
    ...
    for j in range(242):
        x.append(output[i, j*5+2])
        y.append(output[i, j*5+1])
...
for i in range(5):
    ...
    for j in range(242):
        x.append(output[(i+1)*5-1, j*5+2])
        y.append(output[(i+1)*5-1, j*5+1])
